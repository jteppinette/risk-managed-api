...
class ModelTests(TestCase):
    ...
    def test_guest_registration_creates_thumbnail(self):
        """
        Assert that after saving a `GuestRegistration` object, a thumbnail is
        ...
        josh_registration = GuestRegistration.objects.create(identity=josh,
                                                             event=event_one,
                                                             image=relative_file_path)
        self.assertEquals(josh_registration.thumbnail.height, 80)

    def test_guest_registration_creates_images_with_correct_names(self):
        """
        Assert that after saving a `GuestRegistration` object, a thumbnail is
        created from the `image`.
        """
        ...
        relative_file_path = os.path.join('images', 'guests', str(josh.id) + '-' + str(event_one.id) + '-image.jpg')
        copyfile(os.path.join(settings.ROOT, 'static', 'tests', 'test_image.jpg'), os.path.join(settings.MEDIA_ROOT, relative_file_path))
        josh_reg = GuestRegistration.objects.create(identity=josh,
                                                    event=event_one,
                                                    image=relative_file_path)
        self.assertEquals(josh_reg.image.name,
                          'images/guests/' +str(josh.id)+ '-' +str(event_one.id)+ '-image.jpg')
        self.assertEquals(josh_reg.thumbnail.name,
                          'images/guests/' +str(josh.id)+ '-' +str(event_one.id)+ '-thumbnail.jpg')
