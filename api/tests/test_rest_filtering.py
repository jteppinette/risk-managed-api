"""
Asserts that the Water Dragon rest filtering is working properly.
...
class RestFilteringTests(APITestCase):
    """
    The following tests have been created to assert that the rest 
    filtering is functioning.
    """
    ...
    def test_filter_organization(self):
        """
        Assert that the user can make an icontains search on an `Organization`
        name and an exact search on it's `id`.
        """
    ...
    def test_filter_university(self):
        """
        Asser that a user can filter the data set properly.
        """
        admin = create_user('josh', 'josh')
        admin.is_superuser = True
        admin.save()
        ...
        # Create `Universities`
        self.client.post('/universities/', {'name': 'Southern Polytechnic State University',
                                            'acronym': 'SPSU',
                                            'state': 'Georgia'})
        self.client.post('/universities/', {'name': 'Georgia Insitute of Technology',
                                            'acronym': 'GT',
                                            'state': 'Georgia'})
        self.client.post('/universities/', {'name': 'University of Georgia',
                                            'acronym': 'UGA',
                                            'state': 'Georgia'})
        self.client.post('/universities/', {'name': 'University of West Georgia',
                                            'acronym': 'UWG',
                                            'state': 'Georgia'})
        self.client.post('/universities/', {'name': 'University of Alabama',
                                            'acronym': 'UA',
                                            'state': 'Alabama'})
        response = self.client.post('/universities/', {'name': 'University of North Georgia',
                                            'acronym': 'UNG',
                                            'state': 'Georgia'})
    ...
    def test_filter_host(self):
        """
        Assert that a user can filter the data set properly.
        """
        admin = create_user('josh', 'josh')
        admin.is_superuser = True
        admin.save()
        ...
        response = self.client.post('/administrators/',
                                    {'username': 'spsu',
                                     'password': 'spsu',
        ...
        # Create `Nationals`
        response = self.client.post('/nationals/',
                                    {'username': 'ks',
                                     'password': 'ks',
        ...
        # Create `Hosts`
        response = self.client.post('/hosts/',
                                    {'username': 'ksspsu',
                                     'password': 'ksspsu',
                                     'organization': str(ks['id']),
                                     'university': str(spsu['id']),
        ...
        saegt = json.loads(response.content)
    ...
    def test_filter_addresses(self):
        ...
        response = self.client.post('/carboncopyaddresses/',
                                    {'email': 'bob@bob.com'})
        email_two = json.loads(response.content)
    ...
    def test_filter_identities(self):
        """
        Assert that all users can filter `Identities` on all of it's fields.
        """
        admin = create_user('josh', 'josh')
        admin.is_superuser = True
        admin.save()
    ...
    def test_filter_flags(self):
        """
        Assert that all users can filter `Flags` on all of it's fields.
        """
        admin = create_user('josh', 'josh')
        admin.is_superuser = True
        admin.save()
        ...
        response = self.client.post('/flags/',
                                    {'identity': josh['id'],
                                     'violation': 'Stealing'})
        josh_flag_one = json.loads(response.content)
        self.assertEquals(response.status_code, status.HTTP_201_CREATED)

        response = self.client.post('/flags/',
                                    {'identity': olivia['id'],
                                     'violation': 'Underage Drinking'})
        olivia_flag_one = json.loads(response.content)
        self.assertEquals(response.status_code, status.HTTP_201_CREATED)
        ...
        response = self.client.post('/flags/',
                                    {'identity': bobby['id'],
                                     'violation': 'Stealing'})
        bobby_flag_one = json.loads(response.content)
        self.assertEquals(response.status_code, status.HTTP_201_CREATED)
        ...
        response = self.client.post('/flags/',
                                    {'identity': joe['id'],
                                     'violation': 'Stealing'})
        joe_flag_one = json.loads(response.content)
        self.assertEquals(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_filter_events(self):
        ...
        response = self.client.post('/events/', data)
        self.assertEquals(response.status_code, status.HTTP_201_CREATED)
        event_two = json.loads(response.content)
    ...
    def test_filter_procedures(self):
        ...
        ksspsu_proc_one = Procedure.objects.create(description='Interior Sweep',
                                                   event=event)

        event = Event.objects.get(host__id=saespsu['id'])
        saespsu_proc_one = Procedure.objects.create(description='Interior Sweep',
                                                    event=event)
        
        event = Event.objects.get(host__id=ksgt['id'])
        ksgt_proc_one = Procedure.objects.create(description='Interior Sweep',
                                                 event=event)
    ...
    def test_filter_invitees(self):
        ...
        # Create `Procedures`
        event = Event.objects.get(host__id=ksspsu['id'])
        ksspsu_invitee_one = Invitee.objects.create(first_name='Josh',
                                                    last_name='Eppinette',
                                                    gender='Male',
                                                    event=event)

        ksspsu_invitee_two = Invitee.objects.create(first_name='Bobby',
                                                    last_name='Brown',
                                                    gender='Male',
                                                    event=event)

        ksspsu_invitee_three = Invitee.objects.create(first_name='Olivia',
                                                      last_name='Eppinette',
                                                      gender='Female',
                                                      event=event)

        event = Event.objects.get(host__id=saespsu['id'])
        saespsu_invitee_one = Invitee.objects.create(first_name='Josh',
                                                    last_name='Smith',
                                                    gender='Male',
                                                    event=event)
        
        event = Event.objects.get(host__id=ksgt['id'])
        ksgt_invitee_one = Invitee.objects.create(first_name='Josh',
                                                    last_name='Brown',
                                                    gender='Male',
                                                    event=event)
